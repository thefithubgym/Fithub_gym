...
import { Gender, PaymentMethod, MembershipStatus } from "@prisma/client";
import { z } from "zod";
...
const memberProfileFields = {
  ...
  phone: z.string().regex(/^\+?[1-9]\d{1,14}$/, "Please enter a valid phone number (e.g. +919876543210)").min(10, "Phone number must be at least 10 digits"),
...
export const createSingleMemberSchema = z.object({
  ...memberProfileFields,
  // Plan selection
  membershipPlanId: z.string().uuid("Invalid plan selected").optional().or(z.literal("")),
...
export const createCoupleMemberSchema = z.object({
  ...
  // Plan selection
  membershipPlanId: z.string().uuid("Invalid plan").optional().or(z.literal("")),
  customPlanName: z.string().optional(),
  amount: z.coerce.number().min(0, "Amount cannot be negative"),
  registrationFee: z.coerce.number().min(0, "Registration fee cannot be negative"),
  paymentMethod: z.nativeEnum(PaymentMethod),
